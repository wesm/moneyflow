...
class TestViewRefresh:
    ...
    async def test_refresh_view_updates_table(self, controller, mock_view):
        ...
        assert update["column_count"] == 8  # Merchant, Count, In, Out, Net, %, Top Category, Flags
...
class TestViewModes:
    ...
    async def test_merchant_view(self, controller, mock_view):
        ...
        update = mock_view.get_last_table_update()
        assert update["column_count"] == 8
        # Columns should be: Merchant, Count, In, Out, Net, %, Top Category, Flags
        assert update["columns"][0]["key"] == "merchant"
        assert update["columns"][1]["key"] == "count"
        assert update["columns"][2]["key"] == "amount_in"
        assert update["columns"][3]["key"] == "amount_out"
        assert update["columns"][4]["key"] == "total"
        assert update["columns"][5]["key"] == "pct"
        assert update["columns"][6]["key"] == "top_category_display"
        assert update["columns"][7]["key"] == "flags"
    ...
    async def test_category_view(self, controller, mock_view):
        ...
        update = mock_view.get_last_table_update()
        assert update["column_count"] == 7  # Category, Count, In, Out, Net, %, Flags
