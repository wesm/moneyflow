...
func TestActionRegistryMatchesReadOnlyContract(t *testing.T) {
	...
	want := []struct {
		...
		{app.ActionFindDuplicates, []string{"D"}, "D", "Find duplicate transactions", "Views", app.ScopeAnalytical, true, false},
		...
		{app.ActionDeleteTransaction, []string{"x"}, "x", "Delete transaction (with confirmation)", "Actions", app.ScopeAnalytical, true, false},
