...
func TestActionRegistryMatchesReadOnlyContract(t *testing.T) {
	...
	want := []struct {
		...
		{app.ActionFindDuplicates, []string{"D"}, "D", "Find duplicate transactions", "Views", app.ScopeAnalytical, false, true},
...
func TestActionRegistryIsUniqueAndDefensivelyCopied(t *testing.T) {
	...
	for _, definition := range actions {
		...
		if !definition.Web {
			assert.Contains(t, []app.ActionScope{app.ScopeOverlay, app.ScopeLifecycle}, definition.Scope)
		}
	}
