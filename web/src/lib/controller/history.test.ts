...
describe('owned browser history ledger', () => {
  ...
  it('accepts only exact owned history-state shapes', () => {
    ...
    expect(ledger.read({ ...state(0, 'v=1'), revision: '7' })).toBeUndefined()
    expect(ledger.read({ ...state(0, 'v=1'), pending: { active_operations: 1 } })).toBeUndefined()
    expect(ledger.read({ ...state(0, 'v=1'), mutation_token: 'secret' })).toBeUndefined()
