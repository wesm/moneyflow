...
| --- | --- | --- |
| MCP daily workflow | Category/merchant staging, hide, deletion, explicit commit, refresh, and batch recovery exist on the 2026-07-28 contract. Export and taxonomy management have no MCP tools. | Exercise the intended daily workflow through a real MCP client; add the remaining required tools or explicitly accept use of TUI/web for the remainder. |
