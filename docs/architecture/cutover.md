...
| --- | --- | --- |
| MCP daily workflow | Category/merchant staging, local/Amazon taxonomy management, hide, deletion, explicit commit, refresh, batch recovery, and full committed server-side export exist on the 2026-07-28 contract. Filtered/download exports still use TUI/web. | Exercise the intended daily workflow through a real MCP client; add the remaining required tools or explicitly accept use of TUI/web for the remainder. |
