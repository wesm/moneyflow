...
| --- | --- | --- |
| MCP daily workflow | Category staging, explicit commit, refresh, and batch recovery exist. Merchant edits, hide, deletion, and export have no MCP tools. | Exercise the intended daily workflow through a real MCP client; add the required service-backed tools or explicitly accept use of TUI/web for the remainder. |
