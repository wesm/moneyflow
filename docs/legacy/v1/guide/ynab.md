...
Stop Moneyflow and back up the affected profile before changing its credentials. Do not delete
the shared `~/.moneyflow` directory: it can also contain Go profiles. Prefer the profile's reset
workflow; a forgotten password cannot decrypt the old vault.

```bash
# Stop Moneyflow and back up your data. Do not remove the shared configuration root.
uvx --from 'moneyflow==0.11.1' moneyflow
```
...
Credentials encrypted locally. Data only sent to YNAB via official API. See [Security Documentation](https://github.com/wesm/moneyflow/blob/v0.11.1/SECURITY.md).
